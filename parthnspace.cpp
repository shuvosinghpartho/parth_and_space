...
struct Asteroid { float x, y, spd, rot, rotSpd, r; bool on; float flashTimer; };
struct PowerUp  { float x, y, vy; PowerUpType type; bool on; float animT, bob; };
struct EnemyBullet { float x, y, vx, vy; bool on; int age; };
...
void drawShip(float x, float y, float ang, float sc) {
    ...
    glColor4f(wr,wg,wb,0.5f); mpc(-11,-28,5); mpc(11,-28,5);
...
void drawShield(float x, float y) {
    ...
    glColor4f(0.15f,0.65f,1.0f,0.08f+0.05f*p); mpcFill((int)x,(int)y,40);
...
void drawEnemy(const Enemy& e) {
    ...
    glColor3f(er,eg+0.3f*pulse,eb*0.5f); mpcFill(0,0,7);
    glColor4f(er,eg+0.5f,eb,0.4f*pulse); mpc(0,0,10); mpc(0,0,11);
    ...
    dda(-18,18,-26,28); dda(18,18,26,28); bres(-8,12,8,12);
...
void drawBullet(const Bullet& b) {
    ...
    if (b.isHoming) {
        glColor4f(0.2f,1.0f,0.4f,0.3f*pulse); mpc(bx,by,10); mpc(bx,by,11);
    }
    ...
    for (int r = 9; r >= 4; r--) {
        glColor4f(0.0f,0.8f+0.2f*pulse,1.0f,0.18f*(1-(r-4)/5.0f)*pulse); mpc(tx2,ty2,r);
    }
...
void drawPowerUp(const PowerUp& p) {
    ...
    glColor4f(1.0f,1.0f,1.0f,0.9f); txtC(px,py-3,label,GLUT_BITMAP_HELVETICA_12);
}
...
void drawBoss() {
    ...
    glColor3f(cr,cg+0.3f*pulse,cb); mpcFill(bx,by,14);
    glColor4f(1.0f,1.0f,1.0f,0.7f+0.3f*pulse); mpcFill(bx,by,7);
    glColor4f(cr,cg+0.5f,cb,0.5f*pulse); mpc(bx,by,18); mpc(bx,by,19);
    ...
    for (int i = 0; i < 4; i++) {
        ...
        glColor4f(cr,cg+0.6f,cb,0.4f*pulse); mpc(ox,oy,8); mpc(ox,oy,9);
    }
    ...
    glColor3f(cr,cg+0.5f,cb); txtC(bx,barY+12,"BOSS",GLUT_BITMAP_HELVETICA_12);
...
void updateDrawHitRings() {
    for (auto& h : hitRings) {
        ...
        mpc((int)h.x,(int)h.y,(int)h.r); mpc((int)h.x,(int)h.y,(int)h.r+1);
    }
}
...
void drawRadar() {
    ...
    bres(rx-rw/2,ry-rh/2,rx+rw/2,ry-rh/2); bres(rx-rw/2,ry+rh/2,rx+rw/2,ry+rh/2);
    bres(rx-rw/2,ry-rh/2,rx-rw/2,ry+rh/2); bres(rx+rw/2,ry-rh/2,rx+rw/2,ry+rh/2);

    glColor4f(0.3f,0.6f,1.0f,0.7f); txt(rx-rw/2+2,ry+rh/2-10,"RADAR",GLUT_BITMAP_HELVETICA_12);
    ...
    for (auto& a : ast) {
        ...
        glBegin(GL_POINTS); glVertex2f(rx-rw/2+2+(a.x/W)*(rw-4), ry-rh/2+2+(a.y/H)*(rh-4)); glEnd();
    }
...
void drawCombo() {
    ...
    if (comboCount>=5)  { sr=1.0f; sg=0.3f; sb=1.0f; }
    if (comboCount>=10) { sr=1.0f; sg=0.1f; sb=0.1f; }

    glPushMatrix();
    glTranslatef(W/2,H/2+105,0); glScalef(pop,pop,1); glTranslatef(-W/2,-H/2-105,0);
...
void drawHUD() {
    ...
    glColor4f(0.5f,0.8f,0.4f,0.7f); txt(18,H-50,"HI:"+to_string(highScore),GLUT_BITMAP_HELVETICA_12);
    ...
    glColor3f(0.7f+wf*0.3f,0.9f,1.0f); txtC(W/2,H-38,to_string(wave),GLUT_BITMAP_TIMES_ROMAN_24);
    string zoneNames[] = {"DEEP SPACE","ASTEROID BELT","NEBULA ZONE","BOSS ARENA"};
    glColor4f(0.6f,0.8f,1.0f,0.5f); txtC(W/2,H-50,zoneNames[currentZone],GLUT_BITMAP_HELVETICA_12);
    ...
    if (shieldCool > 0) {
        ...
        glBegin(GL_QUADS);
            glVertex2f(20,4); glVertex2f(20+140*prog,4); glVertex2f(20+140*prog,20); glVertex2f(20,20);
        glEnd();
        glColor4f(0.4f,0.5f,1.0f,0.7f); txt(22,8,"SHIELD",GLUT_BITMAP_HELVETICA_12);
    } else {
        float shp = 0.6f+0.4f*sinf(fc*0.07f);
        glColor3f(0.2f,0.85f*shp,0.9f*shp); txt(10,8,"[S] SHIELD READY",GLUT_BITMAP_HELVETICA_12);
    }
    ...
    auto drawPUIcon = [&](int cd, float r, float g, float b, const string& lbl) {
        ...
        glBegin(GL_QUADS);
            glVertex2f(iconX,3); glVertex2f(iconX+30,3); glVertex2f(iconX+30,22); glVertex2f(iconX,22);
        glEnd();
        glColor4f(r,g,b,0.6f);
        glBegin(GL_QUADS);
            glVertex2f(iconX,3); glVertex2f(iconX+30*prog,3); glVertex2f(iconX+30*prog,22); glVertex2f(iconX,22);
        glEnd();
    ...
    bres(0,H,18,H); bres(0,H,0,H-18); bres(W,H,W-18,H); bres(W,H,W,H-18);
    bres(0,28,18,28); bres(0,28,0,46); bres(W,28,W-18,28); bres(W,28,W,46);
}
...
void drawSlowMoOverlay() {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        glBegin(GL_LINE_LOOP);
            glVertex2f(m,m); glVertex2f(W-m,m); glVertex2f(W-m,H-m); glVertex2f(m,H-m);
        glEnd();
    }
    if ((fc/8)%2 == 0) {
        glColor4f(0.6f,0.3f,1.0f,0.7f); txtC(W/2,H/2+150,"— SLOW MOTION —",GLUT_BITMAP_HELVETICA_12);
    }
}
...
void drawSplash() {
    ...
    if (t > 0.15f) {
        ...
        for (auto& s : stars) {
            ...
            glColor3f(tw,tw,tw*1.1f); glPointSize(s.sz);
            glBegin(GL_POINTS); glVertex2f(s.x,s.y); glEnd();
        }
        glPointSize(1);
    }
...
void drawMenu() {
    ...
    bres(140,470,760,470); bres(140,535,760,535); bres(140,470,140,535); bres(760,470,760,535);

    glColor4f(0.5f,0.85f,1.0f,0.8f);
    bres(140,535,165,535); bres(140,535,140,510); bres(760,535,735,535); bres(760,535,760,510);
    bres(140,470,165,470); bres(140,470,140,497); bres(760,470,735,470); bres(760,470,760,497);

    txtWave(W/2,518,"PARTH & SPACE",0.35f+0.2f*gp,0.8f+0.15f*gp,1.0f,3.0f,menuAnim*0.06f,GLUT_BITMAP_TIMES_ROMAN_24);
    glColor3f(0.5f,0.7f,0.9f); txtC(W/2,478,"ULTIMATE EDITION  —  2D SPACE SHOOTER",GLUT_BITMAP_HELVETICA_12);
    glColor4f(0.8f,0.7f,0.2f,0.8f); txtC(W/2,462,"HIGH SCORE: "+to_string(highScore),GLUT_BITMAP_HELVETICA_18);
    ...
    bres(310,415,590,415); bres(310,445,590,445); bres(310,415,310,445); bres(590,415,590,445);
    glColor3f(0.5f,0.9f+0.1f*ep,1.0f); txtC(W/2,425,"PRESS  SPACE  TO LAUNCH",GLUT_BITMAP_HELVETICA_18);
    ...
    bres(350,382,550,382); bres(350,408,550,408); bres(350,382,350,408); bres(550,382,550,408);
    glColor3f(0.45f,0.7f,1.0f); txtC(W/2,389,"[ TAB ]  SETTINGS",GLUT_BITMAP_HELVETICA_12);
    ...
    bres(200,280,700,280); bres(200,372,700,372); bres(200,280,200,372); bres(700,280,700,372);
...
void drawSettings() {
    ...
    bres(230,180,670,180); bres(230,540,670,540); bres(230,180,230,540); bres(670,180,670,540);

    glColor3f(0.5f,0.8f,1.0f); txtC(W/2,520,"SETTINGS",GLUT_BITMAP_TIMES_ROMAN_24);
    ...
    for (int i = 0; i < (int)opts.size(); i++) {
        ...
        if (sel) {
            ...
            glBegin(GL_QUADS);
                glVertex2f(245,oy-5); glVertex2f(655,oy-5); glVertex2f(655,oy+22); glVertex2f(245,oy+22);
            glEnd();
        ...
        if (sel && i < 3) {
            ...
            txt(637,oy,">",GLUT_BITMAP_HELVETICA_18); txt(251,oy,"<",GLUT_BITMAP_HELVETICA_18);
        }
    }
...
void drawPauseOverlay() {
    ...
    bres(300,290,600,290); bres(300,430,600,430); bres(300,290,300,430); bres(600,290,600,430);
    glColor3f(0.5f,0.8f,1.0f);
    bres(300,430,318,430); bres(300,430,300,412); bres(600,430,582,430); bres(600,430,600,412);
    bres(300,290,318,290); bres(300,290,300,308); bres(600,290,582,290); bres(600,290,600,308);

    glColor3f(0.85f,0.9f,1.0f); txtC(W/2,408,"PAUSED",GLUT_BITMAP_TIMES_ROMAN_24);
...
void spawnEnemy() {
    ...
    for (auto& e : ene) if (!e.on) {
        ...
        e.maxHp=(wave>=6)?3:(wave>=3)?2:1;
        e.hp=e.maxHp; e.ai=ai; e.aiTimer=0;
        e.tx=sx; e.ty=sy; e.swarmIdx=(int)(&e-ene);
        break;
    }
}
...
void spawnBoss() {
    boss.x=W/2; boss.y=H+80; boss.targetX=W/2; boss.targetY=H-160; boss.vx=boss.vy=0;
    int diffMul = (difficulty==0)?1:(difficulty==2)?3:2;
    boss.maxHp=200+wave*20*diffMul; boss.hp=boss.maxHp; boss.on=true;
    boss.phase=BOSS_PHASE1; boss.phaseTimer=boss.shootCd=0;
    boss.ang=0; boss.angSpd=0.5f; boss.flashTimer=0; boss.entryProgress=0;
    boss.enraged=false; boss.attackPattern=0; boss.patternTimer=0;
    for (int i = 0; i < 4; i++) { boss.orbAng[i]=i*PI/2; boss.orbOn[i]=true; boss.orbHp[i]=3; }
...
void firePlayer() {
    ...
    if (puLaser > 0) {
        for (auto& b : bul) if (!b.on) { b={sx,sy+38,true,true,false,0,0,0}; bcool=cooldown; break; }
        return;
    }
    if (puHoming > 0) {
        float best=1e9, tx2=sx, ty2=sy+200;
        for (auto& e : ene) if (e.on) { float d=dist(sx,sy,e.x,e.y); if(d<best){best=d;tx2=e.x;ty2=e.y;} }
        if (boss.on) { float d=dist(sx,sy,boss.x,boss.y); if(d<best){tx2=boss.x;ty2=boss.y;} }
        for (auto& b : bul) if (!b.on) { b={sx,sy+38,true,false,true,0,tx2,ty2}; bcool=cooldown; break; }
        return;
    }
...
void shootEnemyBullet(float x, float y, float vx, float vy) {
    for (auto& b : ebul) if (!b.on) { b={x,y,vx,vy,true,0}; break; }
}
...
void initGame() {
    ...
    boss.on=false; exps.clear(); playerTrail.clear();
    for (int i = 0; i < MB; i++) bulletTrails[i].clear();

    spawnWave(); state=PLAY; transAlpha=1.0f; transDir=-1;
}
    ...
    for (int i = 0; i < MB; i++) {
        ...
        for (int j = 0; j < 4; j++) {
            ...
            if (dist(b.x,b.y,ox,oy) < 10) {
                b.on=false; spawnHitRing(ox,oy,1.0f,0.6f,0.2f);
        ...
        if (dist(b.x,b.y,boss.x,boss.y) < 50) {
            ...
            boss.flashTimer=8; spawnHitRing(boss.x,boss.y,1.0f,0.4f,0.1f);
            ...
            if (boss.hp <= 0) {
                ...
                score += 1000+wave*100; scoreAnim=1.0f; boss.on=false;
                triggerScreenShake(15,50); spawnPowerUp(boss.x,boss.y);
...
void update() {
    ...
    for (auto& b : ebul) {
        ...
        if (!shieldOn && dist(b.x,b.y,sx,sy) < 22) {
            b.on=false; spawnHitRing(sx,sy,1.0f,0.4f,0.1f); triggerScreenShake(5,10);
            if (--lives <= 0) { saveHighScore(); state=OVER; }
        }
    }
    ...
    for (auto& e : ene) {
        ...
        switch (e.ai) {
            ...
            case AI_TRACKER: {
                ...
                e.x += dx/len*e.spd*0.6f*gameSpeed; e.y += dy/len*e.spd*0.6f*gameSpeed;
            ...
            case AI_SWARM: {
                ...
                e.x += (fx-e.x)*0.025f*gameSpeed; e.y += (fy-e.y)*0.025f*gameSpeed;
            ...
            case AI_KAMIKAZE: {
                ...
                e.x += dx/len*kSpd*gameSpeed; e.y += dy/len*kSpd*gameSpeed;
                e.ang = atan2f(dx,dy)*180/PI;
                break;
            }
        }
    ...
    for (auto& a : ast) {
        ...
        for (int i = 0; i < MB; i++) {
            ...
            spawnHitRing(a.x,a.y,0.8f,0.6f,0.4f); spawnExplosion(a.x,a.y,0.9f,0.7f,0.5f,0.3f);
    ...
    if (!boss.on) {
        ...
        if (!anyActive) {
            for (auto& a : ast) if (a.on) { anyActive=true; break; }
        }
        if (!anyActive) { wave++; if(score>highScore)saveHighScore(); spawnWave(); }
    }
...
void keyDown(unsigned char k, int, int) {
    if (k==27) { if(state==SETTINGS){state=MENU;return;} exit(0); }
